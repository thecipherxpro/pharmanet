...
function PharmacistProfileContent() {
  ...
  const { data: profileData } = useQuery({
    queryKey: ['pharmacist-profile-data', user?.id],
    queryFn: async () => {
      if (!user || isPublicView || !isOwnProfile) return { payroll: null, cards: [] };
      // Use backend function to get data via service role
      const response = await base44.functions.invoke('getPharmacistOwnProfile', {});
      const payroll = response.data?.payroll || null;
      
      // Cards can still be fetched directly as they have proper RLS
      let cards = [];
      try {
        cards = await base44.entities.WalletCard.filter({});
      } catch (e) {
        console.log('No cards found');
      }
      
      return { payroll, cards };
    },
    enabled: !!user && !isPublicView && isOwnProfile,
  });

  const payrollPreference = profileData?.payroll || null;
  const walletCards = profileData?.cards || [];
