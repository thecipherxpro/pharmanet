...
Deno.serve(async (req) => {
  console.log("🚀 generatePayrollInvoice function called");
  ...
  try {
    ...
    console.log("  - Include Deductions:", includeDeductions);
    ...
    if (includeDeductions) {
      ...
      if (taxConfigs.length === 0) {
        console.error("❌ Tax configuration not found for ON province");
        return Response.json({ error: 'Tax configuration not found. Please contact support.' }, { status: 404 });
      }

      const config = taxConfigs[0];
      console.log("✅ Tax config loaded for", config.province, config.tax_year);
      ...
      console.log("ℹ️ No deductions requested");
    }
    ...
    const message = includeDeductions 
      ? `Invoice generated with deductions. Net: $${netAmount.toFixed(2)}`
      : `Invoice generated without deductions. Total: $${grossAmount.toFixed(2)}`;
