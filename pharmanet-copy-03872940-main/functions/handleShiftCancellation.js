...
Deno.serve(async (req) => {
  try {
    ...
    const hoursBeforeStart = Math.floor((shiftDateTime - cancelDateTime) / (1000 * 60 * 60));
    ...
    if (penaltyTotal > 0) {
      ...
      try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
          ...
          off_session: true,
          confirm: true,
          description: `Cancellation penalty for shift at ${shift.pharmacy_name} on ${shift.shift_date}`,
          metadata: {
            shift_id: shiftId,
