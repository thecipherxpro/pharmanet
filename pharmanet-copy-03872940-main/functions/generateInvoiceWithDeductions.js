...
Deno.serve(async (req) => {
  console.log("🚀 generateInvoiceWithDeductions (WITH DEDUCTIONS) called");
  ...
  try {
    ...
    // Get tax config
    console.log("🧮 Computing deductions...");
    ...
    if (taxConfigs.length === 0) {
      console.error("❌ Tax configuration not found");
      return Response.json({ error: 'Tax configuration not found. Please contact support.' }, { status: 404 });
    }

    const config = taxConfigs[0];
    console.log("✅ Tax config loaded:", config.province, config.tax_year);

    // Calculate deductions with safe fallbacks
    const cppRate = config.cpp_rate || 0.0595;
    const cppMax = config.cpp_max || 3754.45;
    const eiRate = config.ei_rate || 0.0163;
    const eiMax = config.ei_max || 1049.12;
    ...
    if (config.federal_brackets && Array.isArray(config.federal_brackets)) {
      ...
      // Fallback: simple 15% federal tax
    ...
    console.log("  - CPP:", deductions.cpp);
    console.log("  - EI:", deductions.ei);
    console.log("  - Federal Tax:", deductions.federal_tax);
    console.log("  - Ontario Tax:", deductions.ontario_tax);
    console.log("  - Total Deductions:", totalDeductions);
    console.log("  - Net Amount:", netAmount);

    // Generate PDF
    console.log("📄 Generating PDF with deductions...");
    const doc = new jsPDF();
    ...
    return Response.json({ 
      ...
      invoice: invoice,
      pdfUrl: file_url,
      message: `Invoice generated with deductions. Net: $${netAmount.toFixed(2)}`
    });

  } catch (error) {
    console.error('❌ ERROR in generateInvoiceWithDeductions:', error.message);
    console.error('Stack:', error.stack);
    return Response.json({ 
      error: error.message || 'Failed to generate invoice with deductions'
    }, { status: 500 });
  }
});